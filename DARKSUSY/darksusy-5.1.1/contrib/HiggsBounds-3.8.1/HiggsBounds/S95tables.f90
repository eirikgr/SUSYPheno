...
!******************************************************************      
 ...
 double precision, parameter :: delta_Mh_LHC=0.0D0 
 ...
 !------------------------------------------  
  character(LEN=4),parameter :: colliders(3) = (/'LEP ','TEV ','LHC7'/)
  !-------------------------------------------  
 ...
 !**********************************************************
  ...
  do i=lbound(S95_t1,dim=1),ubound(S95_t1,dim=1)
    if(S95_t1(i)%particle_x.eq.Hneut)then
     do c=1,ubound(colliders,dim=1)
      if(WhichColliderElement(S95_t1(i)%expt).eq.c)then
  ...
  do i=lbound(S95_t2,dim=1),ubound(S95_t2,dim=1)
   if(S95_t2(i)%particle_x2.eq.Hneut)then
    do c=1,ubound(colliders,dim=1)
     if(WhichColliderElement(S95_t2(i)%expt).eq.c)then
  ...
  do i=lbound(S95_t1,dim=1),ubound(S95_t1,dim=1)
   if(S95_t1(i)%deltax.lt.-0.5D0)then !i.e. deltax has not been set yet
    S95_t1(i)%deltax = delta_x_default(S95_t1(i)%particle_x,WhichColliderElement(S95_t1(i)%expt)) 
   endif
  enddo
   
  do i=lbound(S95_t2,dim=1),ubound(S95_t2,dim=1)
   if(S95_t2(i)%deltax.lt.-0.5D0)then !i.e. deltax has not been set yet
    S95_t2(i)%deltax = delta_x_default(S95_t2(i)%particle_x2,WhichColliderElement(S95_t2(i)%expt)) 
   endif
  enddo  
 ...
 !****************************************************************** 
 function WhichColliderElement(expt)
 ...
 !****************************************************************** 
 function WhichColliderString(expt)
 ...
 !****************************************************************** 
   ...
   WhichColliderString=colliders(WhichColliderElement(expt))
...
!  if(predobs.eq.1) write(*,*) 'Interpolated value = ', interpol
 ...
 !**********************************************************
  ...
  do j=1,npart    
   ...
   if(     (abs(mass(jj)-mass(j)).le.S95_t1(c)%deltax) &
    ...
    case(110212)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(2011025,2011085,2011161,5895,1414,1487,12001,11010,11030,11021)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))
    case(11031)
     call model_likeness(j,S95_t1(c)%id,t,model_like(j),fact(j))     
    case(11034,12006)
  ...
  if(fact(jj).le.vvsmall)then!A  !Higgs jj doesn't contribute - wait until another call of this subroutine before
   ...
   if((WhichColliderString(S95_t1(c)%expt).eq.'LEP'))then!B 
    cfact_t1=sum(fact)
   elseif(S95_t1(c)%particle_x .ne. Hneut)then!B
    ...
    case(7307,5873) 
     ...
     enddo                         
    case(8961,0598,10010,9290,9674,9897,9999,10607)
    case(7081,9166,9483,5586,9642,1266,0432,9891,5285,3935,6087,6170,10212,6223,6299,10583,10798,10596)
    case(2012015)
 ...
 !******************************************************** 
  ...
  case(8961,9465,9290,9713,9674,0598,9897,9998,9999,6008,6096,6183,3233,6229,6304,10606,10806)  
  ...
  case(11003,11014,2577,11024,1489)  
   descrip=' (p p)->h'//j//'+...->W W +... where h'//j//' is SM-like ' //label 
  case(2748,1408,11011,2011163,11022,11032,1488,12008,2011157,2011112,2011135,2012019) 
...
!  double precision,allocatable :: XS_SM_temp(:), BR_SM_temp(:)
  ...
  select case(id)
  case(711,713,721,723,731,733,741,743,5739,10574,6224,6225,6226) !these have a very simple model-likeness test, so we can 
...
!   BR_rat(3) = div(t%BR_hjtautau(j) , t%BR_Htautau_SM(j) ,0.0D0,1.0D0)     
  ...
  case(6096,10606,10806)
!   ns = 5; nb = 6; call initialise_XS_rat_BR_rat
...
!   XS_rat(1) = t%lhc7%XS_hjZ_ratio(j)   
!   XS_rat(2) = t%lhc7%XS_hjW_ratio(j) 
...
!   BR_SM_temp=unset
 ...
 !************************************************************
  ...
  deallocate(S95_t1)
  deallocate(S95_t2)
  deallocate(Exptrange_Mhmin_forSMXS) 
  deallocate(Exptrange_Mhmax_forSMXS) 
  deallocate(Exptrange_Mhmin_forSMdecays) 
  deallocate(Exptrange_Mhmax_forSMdecays) 
