...
def GetInputParsFromSlha(fn_slha):
    ...
    # Read
    for block in slhacode2txt.keys():
    ...
    def DoScenario(s, SUSYpar, opt={}):
        ...
        if 1:  # re-indent
            ...
            if (require): 
                ...
                isokAll = 0
                istuned = {}
                for var in require.keys(): istuned[var] = 0  # init
                hasbeentunedonce = {}
                for var in require.keys(): hasbeentunedonce[var] = 0  # init
                if s.VB>3: print 'DoScenario: require.keys() = %s' %(require.keys())
                while not isokAll and ntries < s.dict['require_ntriesMax']: 
                    ...
                    for var in verbose.keys(): out += verbose[var]
                    # if s.VB: print out
