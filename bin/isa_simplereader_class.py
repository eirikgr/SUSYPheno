...
class isa_simplereader:
    ...
    def DGLegacy(s):  # not to be used in general cases
        ...
        if 'v1' not in freedict: sys.exit('Fatal::isa_simplereader  v1 not in freedict (has %s)' %(freedict.keys()))  # 2013-07-14
        if 'v2' not in freedict: sys.exit('Fatal::isa_simplereader  v2 not in freedict (has %s)' %(freedict.keys()))
    ...
    def DumpScenario(s):  # could actually be method of IsaproScan
        ...
        for iScan in range(s.nScans):
            ...
            for iScen in range(nScen):
                ...
                for mkey in s.dump.keys(): 
                    if s.VB>2: print 'mkey = %s' %(mkey)
                    if s.VB>2: print 's.dump = %s' %(s.dump)
                    for ivar in range(len(s.dump[mkey])):
                        var = s.dump[mkey][ivar]

                        if var not in scen.dict[mkey]: sys.exit("Fatal::isa_simplereader_class  var %s not among allowed ones for dict %s: %s" %(var, mkey, scen.dict[mkey].keys()))
            ...
            for z in zz:
                ...
                for zcode in zcodes:
                    ...
                    if zcode.count('-') == 1: # then have e.g. 101-105    # so e.g. -steerintlist RANGE:101,102,104-109,121  is allowed
                        z0,z1 = zcode.split('-')
                        s.steer[key] += range(int(z0), int(z1)+1)
