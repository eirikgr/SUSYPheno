...
def readSLHA(spcstr, ignorenobr=False, ignorenomass=True):
    """
    ...
    try:
        ...
        if ignorenomass:
            print 'Warning::pyslha_edited No MASS block found: cannot populate particle masses. Returning reduced pyslha object. (Edited behaviour.)'
...
def writeISAWIG(blocks, decays, ignorenobr=False, precision=8):
    ...
    print len(masses)
    print len(decays)
    a = masses.keys(); a.sort(); print a
    b = decays.keys(); b.sort(); print b
    
    assert(len(masses) == len(decays))


    # ===== ===== I ADD/MOVE FUNCTIONS HERE 
    ...
    for i, pid in enumerate(decays.keys()):
        ...
        for i_d, d in enumerate(decays[pid].decays):
            ...
            if len(d.ids) == 3:
                ...
                pass
            print hwid, fmte, d.br, nme
            #decayout += "%d " + fmte + " %d " % (hwid, d.br, nme)
            decayout += "%d "%(hwid) + fmte%d.br + " %d " %(nme) # % (hwid, d.br, nme)
            print decayout
            ...
            ## Squark/Slepton
                ...
                d.ids = sorted(d.ids, key=cmp_bottomlast)   # FAILS
