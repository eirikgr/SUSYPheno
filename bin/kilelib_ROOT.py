...
def ColourRatio2D(hs, hsInfo=[], opt={}, VB=1, meta=0, fillmode=2): 
    ...
    if VB:
        ...
        if VB>0 or x*y-nxReal*nyReal != nzerosum:
            print 'INFO::Colourratio  (x,xx,X):(%i,%i,%i)  (y,yy,Y)=(%i,%i,%i)  totbin: %i   totrealbin: %i    diff: %i   nzerosum: %i  ' %(x,xx,X, y,yy,Y, x*y, nxReal*nyReal, x*y-nxReal*nyReal, nzerosum), 
...
class NextColourStyle:
    def __init__(self, styles=[], Ncol=-1, cset=0, colFirst=1, argtext='', blackfirst=0):
        ...
        self.nextColour = NextColour(cset=cset, Nmax=Ncol, blackfirst=blackfirst)
...
# =============================================================================
class NextColour:
    def __init__(self, cset=0, Nmax=-1, blackfirst=0):

        self.cols = {}
        self.cols[0] = [2,64,209,223,220, 622,227,416, 791,794, 15,18, 217,606]
        if blackfirst: self.cols[0].insert(0,1)
...
def FromDict(mode=['getTGraph2D'], Dict={}, xFromKeyIndex='', yFromKeyIndex='', xyFromKeyIndices=[], FixKeyIndices={}, zKey='', xtit='x', ytit='y', tit='A quantity', zsc=1.0, xoff=0.8, yoff=1.0, xcent=1, ycent=1, VB=0, FORM='%.0f', txtsize=0.02, extrapolateflat={}):
    ...
    for dkey in dkeys:
        ...
        usethis = 1
        for fkey in FixKeyIndices.keys():
