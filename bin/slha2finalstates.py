...
class slha2finalstates: 
    ...
    def GetMotherDict(s, genea, slha1, optD={}):
        ...
        s.geneaLog.Add(genea=genea, S='dump', txt=' len(slha1)=%i   keys=%s' %(len(slha1), slha1.keys()))
    ...
    def MotherDump(s, genea, fn, particles, which=[], optD={}, fnadd=''):
        ...
        if which == []: whichpart = sorted(particles.keys(), key=lambda x: abs(particles[x].mass))
    ...
    def GetOnelegs(s, genea, fn, ID, particles, SUSYinitiators=[]):
        ...
        if SUSYinitiators == []: SUSYinitiators = particles.keys()
        ...
        s.geneaLog.Add(genea=genea, S='debug', txt='particles.keys() = %s' %(particles.keys()))
        ...
        for iPart in range(len(SUSYinitiators1legSorted)):
            ...
            s.geneaLog.Add(genea=genea, S='debug', txt='just built %-4s :  len(Mothers_Onelegs) = %2i  : %s' %(thisMother_Onelegs.name, len(Mothers_Onelegs), Mothers_Onelegs.keys()))
    ...
    def GetTwolegs(s, genea, Mothers_Onelegs, subprocs=[], xsecs={}, fn='fn', ID='ID'):
        ...
        for isubproc in range(len(subprocs)):
            ...
            if p1generic not in Mothers_Onelegs:
                s.geneaLog.Add(genea=genea, S='warn', txt='Warning::GetTwolegs  Skipping subprocess %s since %s not in Mothers_Onelegs.keys(): %s' %(subproctuple, p1generic, Mothers_Onelegs.keys()))
                continue
            if p2generic not in Mothers_Onelegs:
                s.geneaLog.Add(genea=genea, S='warn', txt='Warning::GetTwolegs  Skipping subprocess %s since %s not in Mothers_Onelegs.keys(): %s' %(subproctuple, p2generic, Mothers_Onelegs.keys()))
        ...
        s.geneaLog.Add(genea=genea, S='info', txt="GetTwolegs  number of Twolegs: %i:  %s" %(len(Twolegs), Twolegs.keys()))
        if s.VB>1:
            for pair in Twolegs:
                twoleg = Twolegs[pair]
    ...
    def FindAllFinalstates(s, genea, fn, xsecs={}, ID=''):  # corresponds to DevelopCascade
        ...
        if s.dict['runmode'] >= 2: 
            ...
            if xsecs: 
                sumMothers = SumMothers(twolegs.values(), name=sumID, particles2keep=s.particles2keep)
    ...
    def PutInFlatDict(s, genea, legs, mode, whichlegs=[], npartMax={'L':3,'l':3,'Th':3}, npartMaxCommon=2, npartMin={}, npartMinCommon=0, gteqlt=['>=','=','<='], opt=[], drop=[], delimDecay='_', delim0=':'):
        ...
        for zname in s.particles2keep:
            iterRangeVars[zname] = range(npartMin.get(zname, npartMinCommon), npartMax.get(zname, npartMaxCommon) + 1)
    ...
    def Main(s, genea):
        ...
        if s.fn_xsecs:
            tmpDict = {}
            execfile(s.fn_xsecs, tmpDict)
            s.xsecs = tmpDict[s.xsecs_dictname]
            del tmpDict
            if s.VB>0: 's.xsecs.keys(): %s' %(s.xsecs.keys())
