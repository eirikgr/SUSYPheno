        ...
        if z[iz].startswith('std:'):
            ...
            for zzz in zz:
                ...
                # C1-N1:BLUE
                    ...
                    contours[zzz] = {'yis':105, 'val':range(100,1001,100), 'col':1, 'sty':2, 'wid':2}
                elif zzz == 'mN2range100': 
                    contours[zzz] = {'yis':102, 'val':range(100,1001,100), 'col':1, 'sty':2, 'wid':2}
                elif zzz == 'mN1range50': 
                    contours[zzz] = {'yis':101, 'val':range(50,1001,50), 'col':1, 'sty':2, 'wid':2}
...
def GetVarText(varin, outtype):
    ...
    if varin not in vars[outtype]: 
        print 'Error::GetVarText  Var %s not in known vars ' %(varin, vars[outtype].keys())
        return 'UnknownVarFor'+varin
...
def GetAxisRangesEtcNew(gg, figname_class='', id2=''):
    ...
    for key in zvars.keys(): setup[key] = zvars[key]
    #print zvars
    V1 = zvars['V1']
    V2 = zvars['V2']
    ...
    if 1: # (to stay with the original indentation)
        ...
        if 201 <= yis <= 299 or 10201 <= yis <= 10299: 
            ...
            if yis == 299:
                ...
                fnn = '%s_freeBR' %(fnn0)
                # print 'yis2: ',yis2
                if 'htit' in yis2.keys(): htit = yis2['htit']
                if 'fnnadd' in yis2.keys(): fnn = '%s_%s' %(fnn0, yis2['fnnadd'])
                elif 'fnn' in yis2.keys(): fnn = yis2['fnn']
                ...
                for ibrs in range(len(brs)):
                    ...
                    pParentT = br.pop(0)
                    if pParentT in ipro.br.part.keys(): pParent = ipro.br.part[pParentT]
                    else:
        ...
        if 501 <= yis <= 599: 
            ...
            if yis == 502: 
                ...
                if (M1,M2,mu,yis2) in dict2.keys(): 
            ...
            if yis == 512: # DGemtKat_2ifb
                zkey = (int(M1),int(M2),int(mu),yis2)
                if not zkey in dict2.keys():
                    print 'WINFO: not in dict2: ', zkey
                    return {'status':'continue'}
                if not zkey in dict3.keys():
            ...
            if yis == 520: # polated:  nSR1 = nFilt(input) * accFilt2SR1_mc_polate
                y = 0.
                if 'extpri' in dict2: 
                    if keyM1M2mu in dict2['extpri'].keys(): 
            ...
            if yis == 551:  # tina
                ...
                y = 0.
                subprocs = [999]
                if 'subprocs' in dict3.keys(): subprocs = list(dict3['subprocs'])
                for sub in subprocs: 
            ...
            if yis == 571:  #eirik's 2L-cutflows
                ...
                if keyM1M2mu in dict2.keys(): 
                    ...
                    for yis2b in yis2: #use yis2 as an array
                        if yis2b in dict2[keyM1M2mu].keys(): 
...
def get2DhistM1M2MU(gg, Scan, yis, VB=0, OPTs=[], ret='h', texs=[], sc=1.,  dict2={}, dict3={}, yis2=0, yis3='', flip=0, palette='coolhot1c', drawtext={}, grid={}, dopt='colz', opt={}, hCont=[], exclude=[], filetype='pdf', stdgrid={}, moveoverflow=0, debug=0, iScan=0, dict_free2d={}): 
    ...
    if palette:
        if not ('palettestatus' in gg.keys() and gg['palettestatus'] == 'set'):  # 2012-06-07: found that set_palette slows down. Hack. 
    ...
    if 'binwidth' in gg.keys():    # 2013-07-14 : IT ALSO WORKS FINE (PRODUCES PLOTS) WITH THE STANDARD PROCEDURE
    ...
    print 'owl: ', V1,V2, bins.keys(), nbins.keys()
    ...
    for iSS in range(aScan.N()): 
        ...
        ### Choose x1 and x2
        if ipro.isaout.has_key(v1): x1 = ipro.isaout[v1]  #<-- VARY
        elif ipro.mass.has_key(v1): x1 = ipro.mass[v1]

        if ipro.isaout.has_key(v2): x2 = ipro.isaout[v2]  #<-- VARY
        elif ipro.mass.has_key(v2): x2 = ipro.mass[v2]
...
def GetCanvas(h, meta={}, dopt='colz', htit='', grid={'on':1}, drawtext={}, save=1, texs=[], hCont=[], filetype='pdf', stdgrid={}, epstopdf=True, figdir='fig1', yis=-1, OptDict={}): 
    ...
    if hCont:
        for con in hCont.keys(): hCont[con].Draw('cont3same')
    ...
    if save:
       fnn = 'test'
       if 'fnn' in meta.keys(): fnn = meta['fnn']
